...
import styles from "./ProtocolBento.module.css";
import useWeb3Store from "../../store/useWeb3Store";
import { formatter } from "../../utils/handleFormat";
import { Lock, Users, Bitcoin, CircleDollarSign } from "lucide-react";
...
export default function SlidingCard() {
    ...
    const tvl = contractState && formatter.formatTVLToUSD(contractState.oraclePriceInUsd, contractState.totalWrappedBitcoinCollateralDeposited);
    const totalUsers = users && users.all && users.all.length;
    const totalDeposits = contractState && ethers.formatUnits(contractState.totalWrappedBitcoinCollateralDeposited).split('.')[0];
    const totalDollarsMinted = contractState && formatter.formatDollarsMintedToUSD(contractState.totalBitcoinDollarsMinted);

    const stats = [
        { title: "Total Value Locked", icon: <Lock />, content: `${tvl && tvl.toString()}` },
        { title: "Total Users", icon: <Users />, content: `${totalUsers}` },
        { title: "Total Deposits", icon: <Bitcoin />, content: `${totalDeposits} wBTC` },
        { title: "Total Dollars Minted", icon: <CircleDollarSign />, content: `${totalDollarsMinted} BTCd` }
    ];
    ...
    }, [selectedIndex, autoSlide, stats.length, changeSlide]);
